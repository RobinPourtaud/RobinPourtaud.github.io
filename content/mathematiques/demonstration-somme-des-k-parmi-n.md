...
$\\quicklatex{size=22}\\sum\_{k=0}^n {n \\choose k} x^{k} y^{n-k}=(x+y)^n$

Si nous prenons $x = 1$ et $y=1$, alors obtenons l'égalité :

$\\quicklatex{size=22}\\sum\_{k=0}^n {n \\choose k } 1^k1^{n-k}=(1+1)^n$

Ce qui nous donne :

$\\quicklatex{size=22}\\sum\_{k=0}^n {n \\choose k}= 2^n$
...
Le cardinal de l'ensemble des parties est donc égal à $ \\sum\_{k=0}^n {n \\choose k}$.
